...
export default function EngineerJobs() {
  ...
  const saveSignature = (signatureType) => {
    if (signatureType === 'inspector') {
      setInspectorSignature(inspectorSignatureRef.current.toDataURL())
    } else if (signatureType === 'customer') {
      setCustomerSignature(customerSignatureRef.current.toDataURL())
    }
  }
    ...
    <div className="min-h-screen bg-gray-100 px-4 py-8">
      {engineer ? (
        <div className="w-full bg-white space-y-6">
          ...
          <div className="mb-6">
            <label htmlFor="maintenanceType" className="block mb-2 text-sm font-medium text-gray-700">
              Type of Maintenance
            </label>
            <select
              id="maintenanceType"
          ...
          {questions.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-xl font-semibold text-gray-700">Checklist Questions</h2>
                ...
                {questions.map((q, index) => (
                  <li key={index} className="text-gray-800">
                    <div className="flex flex-col space-y-2">
                      ...
                      {q.input_type === 'yesno' && (
                        <div className="flex items-center space-x-4">
                          ...
                          <input
                            ...
                            onChange={() => handleYesNoToggle(index, 'Yes')}
                          ...
                          <input
                            ...
                            onChange={() => handleYesNoToggle(index, 'No')}
                            className="form-radio text-blue-500"
                          />
                        </div>
                      )}
                      ...
                      {q.input_type === 'image' && (
                        <div className="flex flex-col space-y-2">
                          <label className="text-gray-700">Upload or Capture Image</label>
                          <input
                            type="file"
                            accept="image/*"
                          ...
                          />
                          {imagePreview && (
                            <img src={imagePreview} alt="Preview" className="mt-4 w-full max-w-xs rounded-lg" />
                          )}
                        </div>
                      )}
                    </div>
          ...
          <div className="space-y-4 mt-8">
            <h2 className="text-xl font-semibold text-gray-700">Signatures</h2>
